...
[abstract]
--
This chapter describes Kafka Connect plugins in the APOC Kafka plugin.
--
endif::env-docs[]
...
The APOC Kafka project provides a Kafka Connect plugin that can be installed into the Confluent Platform enabling:
...
described in the xref:database-integration/kafka/consumer.adoc#dlq-table[Dead Letter Queue configuration parameters table], you need to define kafka broker connection properties:
