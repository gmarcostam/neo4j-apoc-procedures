= APOC Kafka - Sink: Kafka -> Neo4j
:environment: apoc.kafka.sink
:id: apoc_kafka_sink

ifdef::env-docs[]
[abstract]
--
This chapter describes the APOC Kafka Sink in the APOC Kafka plugin.
...
The APOC Kafka Plugin provides several means to handle processing errors.
...
| errors.deadletterqueue.context.headers.prefix | prefix-text | common prefix for header entries, e.g. `"__apoc.kafka.errors."` , default: not set
...
[[apoc_kafka_supported_deserializers]]
== Supported Kafka deserializers

The APOC Kafka plugin supports 2 deserializers:
