...
Another alternative is by using https://docs.anthropic.com/en/api/complete[Anthropic API].
[source,cypher]
----
CALL apoc.ml.openai.completion('What color is the sky?', $anthropicApiKey,
{apiType: 'ANTHROPIC'})
----

.Default Anthropic parameters
[%autowidth, opts=header]
|===
|name | description
...
NOTE: currently https://docs.anthropic.com/en/api/messages-streaming[`stream: true`] is not supported, since the result of apoc.ml.openai must be in JSON format
...
Another alternative is by using https://docs.anthropic.com/en/api/messages[Anthropic API].
[source,cypher]
----
CALL apoc.ml.openai.chat([
